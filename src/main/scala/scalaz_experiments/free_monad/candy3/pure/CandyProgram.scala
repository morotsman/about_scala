    ...
    def applyRule(input: Input)(machine: MachineState): Either[Exception, MachineState] = input match {
      case Coin =>
        if (machine.candies == 0) {
          ...
          Left(throw new IllegalStateException("A coin has already been disposed"))
        }
      case Turn =>
        if (machine.candies == 0) {
          Left(throw new IllegalStateException("No candies left"))
        } else if (!machine.locked) {
          Right(machine.copy(locked = true, candies = machine.candies - 1))
        } else {
          Left(throw new IllegalStateException("No coin has been disposed"))
        }
    }
